...
\begin{center}
\small
\textbf{Experimenters:}~Anonymous
\end{center}
\vspace{6pt}

\begin{abstract}
This paper presents a round-robin tournament in which six small open-source language models (3B--4B parameters) compete as autonomous agents in the two-player board game \textit{Sequence}. The game requires spatial reasoning, hand management, tactical placement, and strategic sabotage under imperfect information. Over 135 fully-played games across three independent iterations, IBM Granite~4 (3B) achieves a dominant 80\% win rate, while Meta Llama~3.2 (3B) finishes at 20\%. A strong first-mover advantage is observed (58.5\% of games won by Player~1), and an unbalanced P1/P2 assignment inflates the apparent performance spread. All models run locally on consumer-grade hardware (single NVIDIA RTX~4060 8~GB, AMD Ryzen~5~5600X, 32~GB DDR4), demonstrating that competitive LLM-based gameplay agents are feasible without cloud infrastructure.
\end{abstract}
...
This work investigates how well small open-weight language models ($\leq 4$B parameters) perform as autonomous \textit{Sequence} players. Six models from five vendors are evaluated in a round-robin tournament with three full iterations (135 total games), all executed on a single consumer GPU. The contributions are:
...
To faithfully reproduce \textit{Sequence}'s rules and provide agents with well-structured state representations, we implemented a dedicated game engine from scratch. The game engine is a pure-Python implementation with immutable state management~\footnote{Source: \texttt{Game/sequence/} in the project repository.}. Key properties are:

\begin{itemize}[nosep]
\item \textbf{Board:} $10 \times 10$ grid, 100 positions. The four corners---(0,0), (0,9), (9,0), (9,9)---are designated as wild (XX) and count for either player's sequences. Wild positions cannot be removed by one-eyed Jacks.
\item \textbf{Deck:} 104 cards (two copies of each standard 52-card deck). Jacks serve special functions: Jack of Hearts (JH) and Jack of Diamonds (JD) are two-eyed (wild placement); Jack of Spades (JS) and Jack of Clubs (JC) are one-eyed (opponent chip removal).
\item \textbf{Hand:} Each player holds 5 cards at all times and draws after playing.
\item \textbf{Win condition:} First player to complete \textbf{two} non-overlapping sequences of five consecutive chips. Diagonals count.
\item \textbf{Moves:} Four legal move types---\texttt{place} (standard chip placement on the two board cells matching a card), \texttt{two\_eyed\_jack} (wild placement anywhere), \texttt{one\_eyed\_jack} (remove any opponent chip except wild corners), and \texttt{dead\_card} (pass when both matching board cells are occupied).
\end{itemize}

The engine pre-computes all legal moves for the current player before each turn, including strategic annotations (sequence-completing moves, blocking moves, partial-sequence extensions) via a sliding-window and run-based analysis module~\footnote{\texttt{Game/sequence/analysis.py}}. These pre-computed annotations are passed directly to each agent, enabling models to reason about move quality without performing their own board analysis.
...
With the game environment established, we now describe how each LLM is wrapped into a tournament-ready agent. Each LLM-backed agent follows an identical architecture, differing only in the underlying model served through Ollama~\cite{ollama}. This design ensures that differences in tournament performance reflect model capability rather than structural advantages in the agent implementation.
...
\item An explicit instruction to respond with a JSON object: \texttt{\{"move\_index": N\}} (or including \texttt{"position": [r,c]} for Jack placements on unlisted positions).
\end{enumerate}
...
Because small models occasionally produce malformed or verbose responses, the agent employs a multi-stage parser with cascading fallback rather than relying on a single parsing strategy. The LLM's raw text response is processed as follows:
\begin{enumerate}[nosep,label=(\roman*)]
...
If all stages fail, the agent retries (up to 3 attempts) with the original prompt, appending a misparse warning. After 3 failures, the agent selects a random legal move. The default LLM temperature is 0.3 for all agents.
...
With the agent architecture in place, we turn to the design of the tournament that exercises it. The goal of the experimental setup is to generate enough games for statistically meaningful rankings while remaining feasible on consumer hardware.
...
\item \textbf{Turn limit:} 500 moves per game, after which the game is declared a draw.
...
All games were executed on a single desktop machine:
\begin{itemize}[nosep]
\item \textbf{GPU:} NVIDIA GeForce RTX~4060, 8~GB VRAM (GDDR6)
\item \textbf{CPU:} AMD Ryzen~5~5600X (6-core, 3.7~GHz)
\item \textbf{RAM:} 32~GB DDR4
\item \textbf{Storage:} NVMe SSD
\item \textbf{OS:} Linux (Arch-based)
\end{itemize}

With 2 concurrent games, VRAM usage peaked at approximately 7.5~GB (two $\sim$3B-parameter models loaded simultaneously at 4-bit quantization via Ollama). No out-of-memory errors occurred.
...
We report results in two stages. First, we present aggregate tournament outcomes---overall standings, head-to-head records, first-mover effects, and cross-iteration stability---derived directly from the 135 game outcomes. Second, we turn to per-move behavioral analysis drawn from a companion run with full move logging, which reveals the mechanisms behind the aggregate rankings.
...
Several patterns stand out. Granite~4 achieves two perfect 9--0 sweeps (against Gemma~4 and Llama~3.2). Gemma~4 similarly sweeps Qwen~3.5. The Ministral~3 vs.\ Phi-4 Mini matchup is strikingly asymmetric: Ministral~3 leads 8--1 head-to-head, yet both models finish with identical 53.3\% overall win rates. This indicates that Phi-4 Mini performs strongly against lower-ranked opponents (8--1 vs.\ Gemma and Llama) but struggles specifically against Ministral~3's play style.
...
Move type proportions are similar across models, but the \textit{quality} and timing of sabotage moves diverge significantly. Sabotage (one-eyed Jack) usage differentiates stronger from weaker agents (Figures~\ref{fig:sabotage} and~\ref{fig:sabotage_outcome}). Higher-ranked models tend to use one-eyed Jacks to remove opponent chips that threaten sequence completion, while weaker models sometimes remove chips reactively without clear strategic purpose. Sabotage timing (Figure~\ref{fig:sabotage_timing}) shows that most one-eyed Jacks are played between turns 10--40, aligning with the mid-game phase when sequences begin to materialize.

\begin{figure}[H]
\centering
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{03_sabotage_rates.png}
\caption{Sabotage and wild Jack rates by model.}
\label{fig:sabotage}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{04_sabotage_vs_outcome.png}
\caption{Sabotage rate: winners vs.\ losers.}
\label{fig:sabotage_outcome}
\end{subfigure}
\caption{Sabotage (one-eyed Jack) usage analysis.}
\end{figure}
...
Strategic move choices are meaningful only when they are successfully communicated back to the engine. Figure~\ref{fig:fallback} shows the average number of retry attempts per move. Models with lower parse-failure rates (fewer retries) correlate loosely with higher win rates: Granite~4 averages fewer than 0.1 retries per move, while Llama~3.2 averages over 0.3. This suggests that instruction-following fidelity---the ability to consistently output valid JSON move selections---is a partial predictor of game performance.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{06_fallback_reliability.png}
\caption{Average parse retry attempts per move (lower is more reliable).}
\label{fig:fallback}
\end{figure}
...
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{11_game_lengths.png}
...
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{10_position_winrate.png}
\caption{Win rate as P1 vs.\ P2 per model.}
\label{fig:positionwr}
\end{subfigure}
\caption{Game length and position-based analysis.}
\end{figure}
...
The results paint a consistent picture: within the 3B--4B parameter range, instruction-following quality rather than scale drives gameplay performance, and structural features of the tournament design introduce biases that must be disentangled from true skill differences. We discuss these themes in turn.

\subsection{Granite~4's Dominance}

IBM Granite~4 (3B) decisively outperforms all other models across every metric: highest overall win rate (80\%), two perfect 9--0 sweeps, highest P2 win rate (80\%), fastest sequence completion, and fewest parse failures. The result is notable because Granite~4 is neither the largest model tested (Qwen~3.5 and Gemma~4 have 4B parameters) nor the most recently released. Its instruction-tuning methodology---which emphasizes structured output and tool-use following---may confer an advantage in the constrained JSON-response format required by the agent architecture.

\subsection{Parameter Count vs.\ Performance}

Within the 3B--4B range, parameter count shows no monotonic relationship with win rate. The two 4B models (Gemma~4 and Qwen~3.5) finish at 53.3\% and 33.3\% respectively, straddling the two strongest 3B models (Granite~4 at 80\% and Phi-4 Mini at 60\%). This suggests that instruction-tuning quality, architecture, and training data distribution matter more than raw parameter count for this task at this scale.
...
The lopsided 8--1 head-to-head between Ministral~3 and Phi-4 Mini, despite their identical overall win rates, is the most striking matchup-level finding. Phi-4 Mini dominates weaker opponents (8--1 vs.\ Gemma~4, 8--1 vs.\ Llama~3.2) but collapses against Ministral~3. This pattern resembles ``rock-paper-scissors'' dynamics sometimes observed in game-playing agents and warrants investigation into whether Ministral~3 employs a specific play style (e.g., aggressive sabotage) that Phi-4 Mini fails to counter.

\subsection{Hardware Feasibility}

The entire 135-game tournament completed on a single consumer GPU (RTX~4060, 8~GB) without errors. With two concurrent games, VRAM usage remained within the 8~GB budget for 3B--4B quantized models. Total wall-clock time was approximately 2--3 hours per iteration, or roughly 6--9 hours for the full tournament (estimated from the game runner's per-move timing of 2--5 seconds and average game length of $\sim$40 turns). This demonstrates that competitive LLM-agent benchmarking in board-game settings is accessible without institutional computing resources.

\subsection{Limitations}

Several limitations should be noted. First, the tournament logs for the primary run contain only game-level outcomes (winner/loser), not per-move data. The behavioral analysis (Sections~5.4.1--5.4.7) therefore draws from a companion run that may differ in model versions or exact outcomes. Second, the models were served at default Ollama quantization levels (typically Q4\_K\_M), which may affect reasoning quality relative to full-precision inference. Third, the prompt design and legal-move filtering heuristics were not ablated; different prompt strategies could change the relative ranking. Fourth, only 6 models were tested, limiting the generality of conclusions about small-model game-playing ability. Finally, the 500-turn cap, while never reached, introduces a theoretical truncation bias.
