...
The game engine is a pure-Python implementation with immutable state management~\footnote{Source: \texttt{Game/sequence/} in the project repository.}. Key properties are:
...
The engine pre-computes all legal moves for the current player before each turn, including strategic annotations (sequence-completing moves, blocking moves, partial-sequence extensions) via a sliding-window and run-based analysis module~\footnote{\texttt{Game/sequence/analysis.py}}.

% ==============================================================================
\section{Agent Architecture}
% ==============================================================================

Each LLM-backed agent follows an identical architecture, differing only in the underlying model served through Ollama~\cite{ollama}.
...
The LLM's raw text response is processed by a multi-stage parser with cascading fallback:
\begin{enumerate}[nosep,label=(\roman*)]
...
Table~\ref{tab:h2h} shows the win-loss record of each row model against each column model across 9 games per pairing (3 games $\times$ 3 iterations).
...
Player~1 (first mover) won 79 of 135 games (58.5\%). The remaining 56 games (41.5\%) were won by Player~2. A binomial test against the null hypothesis of 50\% yields $p \approx 0.048$, suggesting a statistically significant first-mover advantage.
...
Table~\ref{tab:iterations} reports per-iteration win counts. Granite~4's performance is stable (11, 13, 12 wins per 15 games) with iteration-2 being the strongest. Most models exhibit modest variance ($\pm 3$ wins across iterations), confirming that 45 games per model provide sufficient statistical power for reliable ranking.
...
Although the current tournament run logged only game-level outcomes, a companion run with full per-move logging (whose outputs populate the \texttt{analysis\_output/} directory) enables richer behavioral analysis. Key findings from those charts are summarized below.
...
Figures~\ref{fig:heatmaps} and~\ref{fig:zones} analyze chip placement patterns. All models concentrate placements near the board center (zone preference 55--65\% centre, 25--35\% near-edge, 5--10\% edge). Granite~4 places slightly more chips in edge and near-edge zones than other models, possibly reflecting a strategy of building sequences along less-contested lanes. The board heatmaps (Figure~\ref{fig:heatmaps}) show that the most frequently occupied cells are the central columns (4--6) and rows (4--6), consistent with the geometry of sequence formation (a 5-in-a-row requirement makes central positions more valuable).
...
Figure~\ref{fig:seqspeed} plots the turn at which each model completes its first sequence. Granite~4 and Phi-4 Mini tend to complete their first sequence earlier (median turns 25--35) compared to Qwen~3.5 and Llama~3.2 (median turns 35--50), consistent with their higher win rates.
